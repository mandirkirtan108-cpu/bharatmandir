export const UI_MESSAGES = Object.freeze({
  ...
  error: Object.freeze({
    default:
      "We couldn't complete this right now. Please try again in a few moments.",
    ...
    permission:
      "You don't have access to this area. Please use an account with the required permission.",

    notFound:
      "We couldn't find what you were looking for. Please return and try another path.",

    load:
      "We couldn't load this information right now. Please try again in a few moments.",

    save:
      "We couldn't save your changes right now. Please review them and try again.",

    upload:
      "We couldn't upload this file right now. Please check the file and try again.",
  ...
  success: Object.freeze({
    ...
    submitted:
      'Thank you. Your details have been received with gratitude.',

    deleted:
      'The item has been removed.',

    bookmarked:
      'This page has been saved for your spiritual journey.',
  }),
});
...
export function friendlyError(
  error,
  fallback = UI_MESSAGES.error.default
) {
  if (!error) {
    return fallback;
  }
  ...
  if (status === 401) {
    return UI_MESSAGES.error.session;
  }

  if (status === 403) {
    return UI_MESSAGES.error.permission;
  }

  if (status === 404) {
    return UI_MESSAGES.error.notFound;
  }
  ...
  if (
    candidate &&
    !TECHNICAL_ERROR_PATTERN.test(candidate)
  ) {
    return candidate;
  }
