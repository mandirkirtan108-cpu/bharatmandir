...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) { setSuccess(true); setTimeout(() => navigate('/', { replace: true }), 2500); }
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width:'100%', maxWidth:440, boxShadow:'0 20px 60px rgba(0,0,0,0.5)',
        ...
        <div style={{ textAlign:'center', marginBottom:32, marginTop:8 }}>
          ...
          <p style={{ color:S, fontSize:13, margin:'4px 0 0', letterSpacing:'.05em' }}>Create your account</p>
        </div>
        ...
        {success && (
          <div style={{
            ...
            color:'#7fffc4', fontSize:14, textAlign:'center',
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ position:'relative' }}>
              ...
              <button type="button" onClick={()=>setShowPass(v=>!v)} style={eye}>{showPass?'🙈':'👁️'}</button>
            </div>
          </div>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <div style={{ position:'relative' }}>
              <input type={showConf?'text':'password'} placeholder="Repeat password" value={form.confirmPassword}
                onChange={set('confirmPassword')} required
                style={{ ...inp, paddingRight:44 }}
                onFocus={e=>e.target.style.borderColor=S4} onBlur={e=>e.target.style.borderColor=S2} />
              <button type="button" onClick={()=>setShowConf(v=>!v)} style={eye}>{showConf?'🙈':'👁️'}</button>
            </div>
          </div>
        ...
        <p style={{ textAlign:'center', marginTop:24, color:W6, fontSize:13 }}>
          ...
          <Link to="/login" style={{ color:S, fontWeight:600, textDecoration:'none' }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
