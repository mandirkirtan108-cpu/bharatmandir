...
export default function AdminLibraryPage() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      const prepareResponse = await fetch(`${API_BASE}/api/admin/books/upload-url`, {
        method: 'POST',
      ...
      if (!prepareResponse.ok) {
        throw new Error(prepared.detail || `Could not prepare upload (HTTP ${prepareResponse.status})`);
      }

      setUploadStage('Uploading PDF directly to Cloudflare R2...');
      const r2Response = await fetch(prepared.upload_url, {
        method: 'PUT',
        headers: prepared.headers || { 'Content-Type': 'application/pdf' },
        body: file,
      });
      if (!r2Response.ok) {
        throw new Error(
          `Cloudflare R2 upload failed (HTTP ${r2Response.status}). Check the R2 bucket CORS policy.`
        );
      }

      setUploadStage('Validating PDF and starting translation...');
      const finalizeResponse = await fetch(`${API_BASE}/api/admin/books/finalize`, {
        method: 'POST',
        headers: { ...auth(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (!finalizeResponse.ok) {
        const detail = Array.isArray(finalized.detail)
          ? finalized.detail.map(item => item.msg || JSON.stringify(item)).join(', ')
          : finalized.detail;
        throw new Error(detail || `Could not finalize upload (HTTP ${finalizeResponse.status})`);
      }

      setMessage('PDF uploaded to Cloudflare R2. Full translations are now processing.');
      ...
      setMessage(
        err instanceof TypeError
          ? 'Upload network request failed. Check Railway and the Cloudflare R2 bucket CORS policy.'
          : err.message
      );
    } finally {
      setBusy(false);
    ...
    <main className="admin-library">
      ...
      <div className="admin-library-grid">
        <form className="upload-card" onSubmit={submit}>
          ...
          <label className="drop"><FileUp size={30}/><strong>Choose text-based PDF</strong><span>Maximum 500 MB. Scanned PDFs require OCR first.</span><input id="book-pdf" type="file" accept=".pdf,application/pdf" onChange={e => setFile(e.target.files[0])}/>{file && <em>{file.name}</em>}</label>
          <button disabled={busy}>{busy ? <><Loader2 className="spin"/> Uploading…</> : 'Upload and translate'}</button>
          {busy && uploadStage && <p className="message">{uploadStage}</p>}
