...
export default function TempleQRPage() {
  ...
  const templeId = temple ? `BM-${(temple.state||'IN').substring(0,2).toUpperCase()}-${String(temple.id).padStart(4,'0')}` : '';
  ...
  if (error || !temple) return (
    <div className="qr-page">
      <div className="error-wrap">
        ...
        <button className="btn-primary" onClick={() => navigate('/')} style={{ marginTop: 16 }}>{t('qr.back_home')}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="qr-page">
      ...
      <div className="qr-full-card">
        ...
        {temple.name_hindi && <div className="qr-full-hindi">{temple.name_hindi}</div>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24, textAlign: 'left' }}>
          {temple.opening_time && <InfoBox label={t('detail.timing')} value={`${temple.opening_time} – ${temple.closing_time}`} />}
          {temple.entry_fee !== null && <InfoBox label={t('detail.entry_fee')} value={temple.entry_fee === 0 ? t('qr.free_entry') : `₹${temple.entry_fee}`} />}
          {temple.nearest_railway && <InfoBox label={t('detail.nearest_railway')} value={temple.nearest_railway} full />}
        </div>
