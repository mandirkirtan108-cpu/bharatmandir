...
// Library API: public catalogue/page reads use `api`; account-specific
// progress, preferences and bookmarks use `userApi`.
export const libraryAPI = {
  getCategories: () => api.get('/api/library/categories'),
  getBooks: (params = {}) => api.get('/api/library/books', { params }),
  getPage: (bookId, pageNumber, language = 'en') =>
    api.get(`/api/library/books/${bookId}/pages/${pageNumber}`, {
      params: { language },
    }),
  getContinueReading: () => userApi.get('/api/library/progress/continue'),
